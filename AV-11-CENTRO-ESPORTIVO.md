# Avaliação de 5 Dias --- Gestão de Centro Esportivo

## Contexto e objetivo

Projete e implemente uma API backend completa para **Gestão de Centro
Esportivo**. A avaliação tem duração de **5 dias**. A solução será
...
User, Court, Sport, Tournament, Team, TeamMember, Match.
...
Quadra não recebe partidas sobrepostas; equipe não enfrenta a si mesma;
resultado respeita estado da partida.
...
Implemente upload de **regulamento ou imagem**. Valide presença, tamanho
e tipo do arquivo. O upload deve estar conectado a uma funcionalidade
real do domínio.

## Integração externa

Utilize `HttpService` para consumir **clima/feriados**.
URL/configurações devem vir do ambiente e erros/timeout precisam ser
